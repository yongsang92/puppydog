...
public class ItemRepoImpl implements ItemRepoCustom {
    ...
    public Page<ItemShowDto> searchByCategory(ItemSearchCondition itemCondition, Pageable pageable) {
        List<ItemShowDto> content = qf.select(
                Projections.bean(ItemShowDto.class, snack.id, snack.name, snack.thumbnail, snack.price, snack.sales))
                .from(snack).limit(pageable.getPageSize()).offset(pageable.getOffset()).fetch();

        JPAQuery<ItemShowDto> countQuery = qf.select(
                Projections.bean(ItemShowDto.class, snack.id, snack.name, snack.thumbnail, snack.price, snack.sales))
                .from(snack);
    ...
    public Page<ItemShowDto> searchByPriceASC(ItemSearchCondition itemCondition, Pageable pageable) {
        List<ItemShowDto> content = qf
                .select(Projections.bean(ItemShowDto.class, snack.id, snack.name, snack.thumbnail, snack.price,
                        snack.sales))
                .from(snack).orderBy(snack.price.asc()).limit(pageable.getPageSize()).offset(pageable.getOffset())
                .fetch();
    ...
    public Page<ItemShowDto> searchByPriceDESC(ItemSearchCondition itemCondition, Pageable pageable) {
        List<ItemShowDto> content = qf
                .select(Projections.bean(ItemShowDto.class, snack.id, snack.name, snack.thumbnail, snack.price,
                        snack.sales))
                .from(snack).orderBy(snack.price.desc()).limit(pageable.getPageSize()).offset(pageable.getOffset())
                .fetch();
    ...
    public Page<ItemShowDto> searchBySales(ItemSearchCondition itemCondition, Pageable pageable) {
        List<ItemShowDto> content = qf
                .select(Projections.bean(ItemShowDto.class, snack.id, snack.name, snack.thumbnail, snack.price,
                        snack.sales))
                .from(snack).orderBy(snack.sales.desc()).limit(pageable.getPageSize()).offset(pageable.getOffset())
                .fetch();
    ...
    public List<WishItemDto> getWishItems(List<Long> ids) {
        ...
        return qf.select(Projections.fields(WishItemDto.class,item.id,item.name,item.thumbnail,item.price, item.discount))
        .from(item)
    ...
    @GetMapping("/snacks")
    public String snack(ItemSearchCondition itemCondition, PageFilter pageFilter, Model model) {

        Pageable pageable = pageFilter.makePageable();
        Page<ItemShowDto> pagingResult = itemRepo.searchByCategory(itemCondition, pageable);
        model.addAttribute("result", new PageMaker<>(pagingResult));
        return "/item/snackList";
